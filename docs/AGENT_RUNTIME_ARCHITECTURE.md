...
For harness observability, the evaluation payload exposes `harness_runtime_counts`, `model_mode_counts`, `tool_policy_counts`, `budget_status_counts`, `schema_status_counts`, and `llm_usage_counts`, derived only from answers with harness metadata.
