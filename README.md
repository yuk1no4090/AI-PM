...
Evaluation metrics are scoped to the requested `projectId`, so safety status, memory status, recent memory events, harness runtime, model mode, tool policy, budget status, schema status, fallback, response-time counts, recent safety events, and recent harness runs reflect the currently selected imported repository. Metrics ignore unknown feedback types so old or manually edited store data cannot pollute quality rates and failure-reason counts.
